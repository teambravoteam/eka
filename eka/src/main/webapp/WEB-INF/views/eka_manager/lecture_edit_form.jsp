...
<script type="text/javascript" src="../resources/manager/custom/js/lecture_edit_form.js"></script>
</head>
<style>
body {
	...
	<div id="wrapper">
		...
		<div id="content-wrapper" class="d-flex flex-column">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">강좌 수정</h1>
          </div>

          <div class="card shadow mb-4">
            <div class="card-body">
              <form action="lecture_edit_update" method="post">
                <table id="lecture_add">
                  <tr>
                    <th>강좌명</th>
                    <td>
                      <input type="text" class="title" name="name" value="${lecture.name}">
                    </td>
                    <th>과목</th>
                    <td>
                    <select id="school" name="schoolcate">
                        <c:forEach var="school" items="${school}" varStatus="status">
                        	<option value="${school.schoolcate}">${school.schoolcate}</option>						
						</c:forEach>
                      </select>
                      <input type="hidden" name="schoolcateinput" value="${lecture.schoolcate}"/>
                      <select id="grade" name="gradecate">
                  ...
                  <tr>
                    <th>기간</th>
                    <td><input type="date" name="startLectureDate" value="${lecture.startLectureDate}">&nbsp~
                    <input type="date" name="finishLectureDate" value="${lecture.finishLectureDate}"></td>
                    <th>시간</th>
                    <td><input type="time" name="startLectureTime" value="${lecture.startLectureTime}">&nbsp~
                    <input type="time" name="finishLectureTime" value="${lecture.finishLectureTime}"></td>
                  </tr>
                  <tr>
                    <th>요일</th>
                    <td>
                      ...
                      <input type="hidden" name="dayinput" value="${lecture.lectureDay}"/>
                    </td>
                    <th>담당강사</th>
                    <td>
					 ...
					 <input type="hidden" name="teacherinput" value="${lecture.teacher}"/>
                    </td>
                  </tr>
                  <tr>
                    <th>정원</th>
                    <td><input type="number" class="number" name="lectureCapacity" value="${lecture.lectureCapacity}">&nbsp명</td>
                    <th>가격</th>
                    <td><input type="number" class="price" name="price" value="${lecture.price}">&nbsp원</td>
                  </tr>
                </table>
